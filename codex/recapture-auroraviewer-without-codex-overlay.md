...
현재 `viewer / thumbnail / compare / color` ko/en 스크린샷에 왼쪽 위 보라색 Codex Computer Use 관찰/제어 표시가 들어가 있다. 이 표시는 AuroraViewer UI가 아니므로 최종 공개 자산에서 완전히 제거한다.

이미지를 후처리하거나 보라색 부분만 지우지 마라. **같은 AuroraViewer 상태를 깨끗한 native window capture로 다시 촬영**한다.
...
4. 가능하면 native screenshot helper를 사용하고, 안정적인 fallback으로 `screencapture -x -l <window-id> <output.png>`를 사용한다.
5. Computer Use 관찰 표시가 계속 영향을 주면 상태 구성 후 Computer Use observation을 종료/해제하고 shell 기반 window capture를 실행한다.

전체 화면 또는 임의 rectangle 캡처를 최종 자산으로 사용하지 마라.

## QA

각 캡처를 저장하기 전에 직접 확인해.
...
최종 8개 파일을 `public/assets/screenshots/`에 반영하고 빌드 산출물도 갱신한다. 실제 이미지 크기가 달라졌으면 HTML width/height metadata를 정확히 맞춘다.

마지막에 production build와 site check를 실행하고 Home/Features에서 모든 새 스크린샷을 확인한다.

완료 보고에는 capture 방식, 최종 크기, ko/en 8장 모두 Codex 표시가 없음을 확인한 결과만 간결하게 남겨.